...
class SideBar extends React.Component {
  render() {
    const {sidebarOpen, clickOutside, isAdmin} = this.props
    const onClickOut = e => {
    ...
    //   )
    // }
    let title = (<h2>MinIO Browser</h2>)
    if (isAdmin) {
      title = (<h2>MinIO Admin Console</h2>)
    }
    ...
    return (
      <ClickOutHandler onClickOut={onClickOut}>
        ...
        >
          ...
          {isAdmin &&
          <div className="fes-list admin-list">
            <AdminAction/>
          </div>
          }
          {isAdmin && <hr/>}
...
const SideBarController = connect(
  mapStateToProps,
  mapDispatchToProps
)(SideBar)

export {SideBarController as SideBar}
